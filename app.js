...
const latestCatalogRunKey = "questLatestCatalogRun";
...
function AnalyzerPage({ catalog, reloadCatalog }) {
  ...
  const [payload, setPayload] = React.useState(() => catalog.latest_catalog_run || readStoredCatalogRun());
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState("");

  function updatePayload(nextPayload) {
    setPayload(nextPayload);
    writeStoredCatalogRun(nextPayload);
  }
  ...
  React.useEffect(() => {
    if (catalog.latest_catalog_run) {
      updatePayload(catalog.latest_catalog_run);
    }
  }, [catalog.latest_catalog_run]);
  ...
  return h(
    ...
    payload
        ? h(
            ...
            h(BulkReviewQueue, { items: mediumReviewItems, reloadCatalog, onUpdate: updatePayload, currentPayload: payload }),
            h(ExceptionReviewQueue, { items: highReviewItems, reloadCatalog, onUpdate: updatePayload, currentPayload: payload })
          )
      : null
  );
}
...
function readStoredCatalogRun() {
  try {
    const value = window.sessionStorage.getItem(latestCatalogRunKey);
...
function writeStoredCatalogRun(payload) {
  try {
    if (payload) {
      window.sessionStorage.setItem(latestCatalogRunKey, JSON.stringify(payload));
    }
  } catch (error) {
    // Ignore storage failures; the backend cache still restores the run.
  }
}
